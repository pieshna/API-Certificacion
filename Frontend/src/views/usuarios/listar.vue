    ...
    <div class="row justify-content-center">
      <div class="col-md-3" v-for="repuesto in displayedRepuestos" :key="repuesto._id">
        <div class="card">
          ...
          <div class="card-body">
            <h5>{{repuesto.username}}</h5>
            <p>Nombre: {{repuesto.name}}</p>
              <p>Correo: {{repuesto.email}}</p>
              <!--<button >Detalles</button><br><br>-->
              <button class="btn btn-danger separar" @click="eliminarItem(repuesto._id)">eliminar</button>
              <router-link class="btn btn-success" v-bind:to="'/usuarios/edit/'+repuesto._id">editar</router-link>
  ...
  methods: {
    async getRepuestos(){
      ...
      async eliminarItem(id){
        try {
            await axios.delete(`${this.baseURL}/user/${id}`,{
                    headers:{
                    'auth-token': this.token,
                    }
                }).then((response)=> {
